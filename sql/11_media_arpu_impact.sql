...
arpu AS (
    SELECT
        ...
        SUM(package_revenue + add_on_revenue) AS media_revenue
    FROM active_customer GROUP BY 1, 2
)
SELECT
    snapshot_month,
    customer_type,
    ...
    ROUND((lease_revenue + media_revenue) / NULLIF(active_ca, 0), 0) AS combined_arpu,
    ROUND(
